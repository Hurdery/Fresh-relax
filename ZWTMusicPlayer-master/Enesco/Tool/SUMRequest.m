...
@property (nonatomic, assign)YTKRequestSerializerType customResponseSerializerType;
             ...
             failuerCompletion:(SUMRequestCompletionBlock)failuerCompletion;
{
    if (self = [super init]) {
        ...
        self.customResponseSerializerType = YTKRequestSerializerTypeHTTP;
...
{
    SUMRequest *sumRequest = [[SUMRequest alloc]initWithApiName:apiName
                                                      params:params
    ...
    [sumRequest startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
        ...
        if ([request isKindOfClass:[SUMRequest class]]) {
            SUMRequest *sRequest = (SUMRequest *)request;
    ...
    [sumRequest startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
        ...
        if ([request isKindOfClass:[SUMRequest class]]) {
            ...
            if (sRequest.resultInfo) {
                if ([SUMUser shareUser].tokenInfo.errorMsg.length>0) {
