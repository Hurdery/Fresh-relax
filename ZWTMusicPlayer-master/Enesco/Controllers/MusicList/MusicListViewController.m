...
- (void)handleTapIndicator {
    ...
    if (musicVC.musicEntities.count == 0) {
        [self showMiddleHint:@"No playing songs"];
        return;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *musicListCell = @"musicList";
    MusicEntity *music = _musicEntities[indexPath.row];
    MusicListCell  * collectCell = [tableView dequeueReusableCellWithIdentifier:musicListCell];
    if (!collectCell) {
        collectCell = [[MusicListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:musicListCell];
    }
      collectCell.musicNumber = indexPath.row + 1;
        collectCell.musicEntity = music;
//        collectCell.delegate = self;
//        [self updatePlaybackIndicatorOfCell:collectCell];
