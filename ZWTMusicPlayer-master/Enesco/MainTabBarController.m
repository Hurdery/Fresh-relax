...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(goToLoginViewController) name:kNotificationNameForPushToLoginViewController object:nil];
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(logout) name:kNotificationNameForLogOut object:nil];
...
-(void)loginTimeOutGoToLoginViewController
{
    if (!self.presentedViewController) {
        ...
        [SVProgressHUD way_showInfoCanTouchWithStatus:[SUMUser shareUser].tokenInfo.errorMsg dismissAfterInterval:1.5 onView:self.view];
    }
...
-(void)queryDomainInfo
{
    ...
    */
    ...
    NSString *paramsString = [NSString encryptedByGBKAES:[paramsDic JSONString]];
    [SUMRequest startWithDomainString:CPSerVerAPINameForAPIMain
                              apiName:@""
          ...
          if (request.resultIsOk) {
              ...
              if (domainUrl) {
                  ...
                  [CPGlobalDataManager shareGlobalData].domainUrlString = domainString;
                  [CPGlobalDataManager shareGlobalData].updateUrl = updateUrl;
                  [CPGlobalDataManager shareGlobalData].lowVersion = lowVersion;
                  [CPGlobalDataManager shareGlobalData].currentVersion = currentVersion;
