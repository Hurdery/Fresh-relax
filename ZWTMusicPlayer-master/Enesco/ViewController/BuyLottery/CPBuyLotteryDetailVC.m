...
#import "CPBuyLotterySectionView.h"
#import "CPSelectedOptionsView.h"
#import "CPBuyCustomLotteryConfirmView.h"
#import "CPBuyFastLotteryConfirmView.h"
#import "CPBuyLotteryFastItem.h"
#import "CPBetRecordVC.h"
#import "CPBuyLetterLotterySectionView.h"
...
@interface CPBuyLotteryDetailVC ()<CPBuyLotterySectionViewDelegate,CPBuyLetterLotterySectionViewDelegate,UIScrollViewDelegate>
{
    ...
    CPLotteryShowResultView *_resultView;
...
- (void)viewDidLoad {
    ...
    [CPBuyLotteryManager shareManager].currentBuyLotteryType = resultType;
                        ...
                        errorMsg:(NSString **)errorMsg
{
    ...
    *isBet = NO;
    ...
    for (CPBuyLetterLotterySectionView *sectionView in _sectionViews) {
                             ...
                             errorMsg:(NSString **)errorMsg
{
    ...
    *isBet = NO;
    ...
    for (CPBuyLotterySectionView *sectionView in _sectionViews) {
        ...
        if ([sectionView isSelectedLianXiaoWeiLotterys]) {
            *isBet = YES;
            if (![sectionView isVerifySelectedLianXiaoWeiLotterys]) {
                ...
                [self.lianXiaoWeiBetInfos addObjectsFromArray:[sectionView betValuesInfo]];
            }
        }
    }
...
-(void)setSelectedPlayKindIndex:(NSInteger)selectedPlayKindIndex
{
    ...
    [CPBuyLotteryManager shareManager].currentPlayKindDes = self.playKindNameList[_selectedPlayKindIndex];
}
    ...
    switch (sender.tag) {
        ...
        case 22:
        {
            ...
            if (resultType == CPLotteryResultForXGLHC || resultType == CPLotteryResultForPCDD) {
                ...
                AppDelegate *app = (AppDelegate *)[UIApplication sharedApplication].delegate;
                NSString *gid = [_playInfo DWStringForKey:@"num"];
                [app.maiTabBarController goToTrendViewControllerWithGid:gid];
            }
        ...
        case 33:
        {
            ...
            if ([playKindName isEqualToString:@"合肖"]) {
                ...
                for (CPBuyLotteryFastItem *item in self.heXiaoBetLotteryInfos) {
                ...
                [CPBuyLotteryManager shareManager].currentBetPeriod = [self.playInfo DWStringForKey:@"period"];
                [CPBuyFastLotteryConfirmView showFastLotteryConfirmViewOnView:self.navigationController.view lotterys:@[betInfo] numberPeriods:[CPBuyLotteryManager shareManager].currentBetPeriod specailType:1 comfirm:^(BOOL isConfirm, NSString *value) {
                    if (isConfirm) {
                ...
                CPBuyLetterLotterySectionView *sectionView = _sectionViews[0];
                ...
                if ([self checkLianMaVerifyAndIsBet:&isBet errorMsg:&errorMsg]) {
                    if (isBet) {
                        if (self.lianMaBetInfos.count>0) {
                            [CPBuyLotteryManager shareManager].currentBetPeriod = [self.playInfo DWStringForKey:@"period"];
                            [CPBuyFastLotteryConfirmView showFastLotteryConfirmViewOnView:self.navigationController.view lotterys:self.lianMaBetInfos numberPeriods:[CPBuyLotteryManager shareManager].currentBetPeriod specailType:2 comfirm:^(BOOL isConfirm, NSString *value) {
                                if (isConfirm) {
                ...
                if ([self checkLianXiaoWeiVerifyAndIsBet:&isBet errorMsg:&errorMsg]) {
                    if (isBet) {
                        if (self.lianXiaoWeiBetInfos.count>0) {
                            [CPBuyLotteryManager shareManager].currentBetPeriod = [self.playInfo DWStringForKey:@"period"];
                            [CPBuyFastLotteryConfirmView showFastLotteryConfirmViewOnView:self.navigationController.view lotterys:self.lianXiaoWeiBetInfos numberPeriods:[CPBuyLotteryManager shareManager].currentBetPeriod specailType:2 comfirm:^(BOOL isConfirm, NSString *value) {
                                if (isConfirm) {
            ...
            if (self.betType == CPBuyLotteryBetFast) {
                ...
                [CPBuyLotteryManager shareManager].currentBetPeriod = [self.playInfo DWStringForKey:@"period"];
                [CPBuyFastLotteryConfirmView showFastLotteryConfirmViewOnView:self.navigationController.view lotterys:_betResultDic.allValues numberPeriods:[CPBuyLotteryManager shareManager].currentBetPeriod comfirm:^(BOOL isConfirm, NSString *value) {
                    if (isConfirm) {
                ...
                [CPBuyLotteryManager shareManager].currentBetPeriod = [self.playInfo DWStringForKey:@"period"];
                [CPBuyCustomLotteryConfirmView showCustomLotteryConfirmViewOnView:self.navigationController.view lotterys:_betResultDic.allValues numberPeriods:[CPBuyLotteryManager shareManager].currentBetPeriod comfirm:^(BOOL isConfirm) {
...
-(void)reloadTopViewData
{
    ...
    _resultView = [[CPLotteryShowResultView alloc]initWithFrame:CGRectMake(5, 30, kScreenWidth - 2*5, 0)];
    CGFloat resultViewHeight = [CPLotteryShowResultView resultViewHeightByResult:result resultType:CPLotteryResultTypeByTypeString(resultType) isWaitOpenResult:lastOpen.length==0?YES:NO maxWidth:_resultView.width isList:NO];
...
-(void)reloadContentView
{
    @synchronized (self) {
        ...
        for (UIView *subView in _scrollView.subviews) {
            if ([subView isKindOfClass:[CPBuyLotterySectionView class]] || [subView isKindOfClass:[CPBuyLetterLotterySectionView class]]) {
                [subView removeFromSuperview];
            }
        }
...
//                _sortBetTypeView
                _sortBetTypeView.hidden = YES;
                CPBuyLetterLotterySectionView *sectionView = [[CPBuyLetterLotterySectionView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, 0) selectMaxCount:11 sectionName:@"自选不中" delegate:self];
...
//                lianMaLotteryInfos
                ...
                for (int i = 0; i<self.lianMaLotteryInfos.count; i++) {
                    ...
                    CPBuyLetterLotterySectionView *sectionView = [[CPBuyLetterLotterySectionView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, 0) selectMaxCount:maxCount selectMinCount:minCount sectionName:name delegate:self];
...
//                _topView.height = 160.0f-_sortBetTypeView.height;
                ...
                for (int i = 0; i<mPlayDetailList.count; i++) {
                    ...
                    CPBuyLotterySectionView *sectionView = [[CPBuyLotterySectionView alloc]initWithFrame:CGRectMake(0, 0,kScreenWidth, 0) buySectionInfo:info betType:CPBuyLotteryBetForLianXiaoWei delegate:self];
                    [_scrollView addSubview:sectionView];
                    lastView = sectionView;
                    [_sectionViews addObject:sectionView];
                }
            ...
            if (([playKindName rangeOfString:@"特码"].length>0 || [playKindName isEqualToString:@"正码"] || ([playKindName rangeOfString:@"正"].length>0&&[playKindName rangeOfString:@"特"].length>0))&& _betType == CPBuyLotteryBetCustom){
                ...
                for (int i = 0; i<playDetailList.count; i++) {
                    ...
                    CPBuyLotterySectionView *sectionView = [[CPBuyLotterySectionView alloc]initWithFrame:CGRectMake(0, 0,kScreenWidth, 0) buySectionInfo:info betType:CPBuyLotteryBetForLHCHorizontalViewStyle delegate:self];
                    [_scrollView addSubview:sectionView];
                    [_sectionViews addObject:sectionView];
                }
...
//        heXiaoLotteryInfo
        [SUMUser shareUser].buyLotteryDetailHasNumberPan = [self isNumberPan];
        CPBuyLotteryBetType betType = specailType==3?CPBuyLotteryBetForHeXiao:_betType;
        UIView *lastView =_topView;
        for (int i = 0; i<playDetailList.count; i++) {
            NSDictionary *info = playDetailList[i];

            CPBuyLotterySectionView *sectionView = [[CPBuyLotterySectionView alloc]initWithFrame:CGRectMake(0, 0,kScreenWidth, 0) buySectionInfo:info betType:betType delegate:self];
            [_scrollView addSubview:sectionView];
            lastView = sectionView;
            [_sectionViews addObject:sectionView];
        }
...
-(void)buyLetterLotteryClickShowDetailAction
{
